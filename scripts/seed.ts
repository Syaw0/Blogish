...
const seedDb = async (redisClient: any, mariaClient: Pool) => {
  ...
  await con.query(
    "CREATE TABLE blogish.users (name varchar(30) not null default('unnamed'),email varchar(120) not null UNIQUE,password char(64) not null,description varchar(50) not null default(''),profileUrl varchar(100) not null default('/prof/default.png'),userId int not null auto_increment primary key)  ;"
  );
  await con.query(
